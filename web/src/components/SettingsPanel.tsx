      ...
      >
        ...
        <section className="space-y-3 border-b border-white/10 pb-5">
          ...
          <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
            {VIDEO_SCENES.map((id) => {
              const s = SCENES[id];
              const selected = bgPrefs.scene === id;
              return (
                <button
                  key={id}
                  type="button"
          ...
          <button
            ...
            className={`relative mt-1 aspect-[3/1] w-full overflow-hidden rounded-lg border ${
              bgPrefs.scene === "map"
                ? "border-[#b8422e] ring-1 ring-[#b8422e]"
                : "border-white/15"
            }`}
