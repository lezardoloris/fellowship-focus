...
export type BlockerModeStatus = {
  ...
  /** Open connect chooser when disconnected. */
  connect?: () => void;
};
...
/** App-wide Shield — the product moat, visible on every tab. */
...
export function usePublishBlockerMode(status: BlockerModeStatus) {
  ...
  const { live, arming, busy, connected, toggle, connect } = status;
  useEffect(() => {
    publish({ live, arming, busy, connected, toggle, connect });
    return () => publish(null);
  }, [publish, live, arming, busy, connected, toggle, connect]);
}
  ...
  return (
    ...
    >
      ...
      <button
        ...
        role="switch"
        aria-checked={live}
        aria-label={arming ? "Shield arming" : `Shield ${live ? "ON" : "OFF"}`}
        disabled={busy || arming || !connected}
        onClick={() => {
          void toggle();
        }}
